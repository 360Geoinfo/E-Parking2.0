...
const handleLogout = async () => {
  try {
    ...
    if (!token) {
      console.log('[Logout] No token found, skipping server call');
    } else {
      // Call the backend to clear token from DB
      const response = await axios.post(
        'http://192.168.102.55:3001/logout',
        {},
        {
          headers: {
      ...
      console.log('[Logout] Server response:', response.data.message);
    }
